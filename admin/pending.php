...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `orde` WHERE ORDE_SELECTFILE IN (SELECT FILE from stat WHERE STAT_STATUS='Pending') AND
    CONCAT(`USER_NAME`,`ORDE_SELECTFILE`,`PAYMENT`) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);

}
 else {
    $query = "SELECT * FROM `orde` WHERE ORDE_SELECTFILE IN (SELECT FILE from stat WHERE STAT_STATUS='Pending')";
    $search_result = filterTable($query);
